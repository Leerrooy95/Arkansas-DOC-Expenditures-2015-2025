# Arkansas Division of Correction Expenditures, FY2015–FY2025

https://zenodo.org/records/17663528

**Total documented spending across ten fiscal years: $4.19 billion**

This repository presents the first complete public compilation of official expenditure figures published by the Arkansas Division of Correction (ADC) and Division of Community Correction (ACC) from FY2015 through FY2025 — extracted from 287 state reports.

**Visuals**
1000004948 (663×330)

### The Key Fact
**FY 2024 total expenditures: $469,289,163**  
— ADC FY24 Annual Report, page 8 
Screenshots in Files as 'img-'

Arkansas spent **nearly the entire cost** of the proposed new 3,000-bed prison **in a single year of operations** — while simultaneously telling legislators and the public that the state “cannot afford to delay” construction due to lack of funds.

### Quick View: Ten-Year Expenditure Trend (ADC’s own numbers)

![Arkansas DOC Expenditures 2015–2025]

| Fiscal Year | Total Expenditures    | Notes |
|-------------|-----------------------|-------|
| 2024        | **$469,289,163**      | More than the entire proposed new prison |
| 2023        | $434,337,302          |       |
| 2022        | $417,506,575          |       |
| 2021        | $392,181,404          |       |
| 2020        | $380,489,377          |       |
| 2019        | $375,058,996          |       |
| 2018        | $366,102,656          |       |
| 2017        | $365,782,232          |       |
| 2016        | $363,825,884          |       |
| 2015        | $359,869,988          |       |
| **Total**   | **$4,192,494,725**    |       |

### “Savings” vs. Reality (FY24)
- Inmate agricultural production: **$5.06 million** (1.1 % of budget)  
- Industry + work-release revenue: **$13.8 million** (2.9 %)  
→ **Taxpayers funded more than 96 %** of the annual total.

### Repository Contents
- `Arkansas_DOC_Financials_FULL_2025.csv` – 29,277 extractions from 287 reports  
- `FY24-Annual-Report-FINAL.pdf` – official source of the $469 million figure  
- `FY24-469M-screenshot.png` – direct screenshot of page 8  
- `sources.txt` – contact for full 8 GB archive of all 287 original PDFs

All data consists of public records published by the Arkansas Department of Corrections.  
No external estimates. No modeling. Only the state’s own numbers.

Prepared by Austin Smith  
U.S. Army 19D Veteran | Former Arkansas Correctional Officer  
November 2025

**License**: CC0 1.0 — public domain. Use and share freely.
